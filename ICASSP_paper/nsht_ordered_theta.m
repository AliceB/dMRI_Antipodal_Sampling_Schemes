...
try
  ...
  load(['../Data/V2_just_enough_samples_minPm_dMRI_ICASSP_' num2str(L)], 'TT_updated', 'min_cond_vec');
  %load(['../Data/equator_offset_dMRI_ICASSP_' num2str(L)], 'TT_updated', 'min_cond_vec');
  %load('nothing');
    ...
    if L==1
            TT_ordered_index = [NUM_RINGS]; %choose the ring closest to the equator
    else
            TT_ordered_index = [NUM_RINGS, 2*NUM_RINGS]; %choose the ring closest to the equator
    end
   min_cond_vec = [];
    %%
    ...
    for m = L-3:-2:0
            ...
            if m~=0
                ...
                TT_ordered_index = [ (ii_vec(II) + NUM_RINGS) ii_vec(II) TT_ordered_index];
            else
                [M,II] =min(cond_vec_1);
    ...
    save(['../Data/V2_just_enough_samples_minPm_dMRI_ICASSP_' num2str(L) '.mat'], 'TT_updated', 'min_cond_vec');
end %% end try/catch
