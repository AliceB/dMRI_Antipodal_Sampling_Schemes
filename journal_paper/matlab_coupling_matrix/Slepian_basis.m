function [Basis_mat no_of_basis Basis_eig_value] = Slepian_basis(L,theta_c)
...
for m=0:1:L-1
    ...
    Gll_mat = Gll_matrix_order(L, m,theta_c );

    [V,D] = eig(Gll_mat);
    for ii=length(D):-1:1
        %if(abs(D(ii,ii))>0.0000000000001)
           temp_spect = V(:,ii); % choose the temp_spect for which eigenvalue >0.7 
           if m==0
               no_of_basis = no_of_basis + 1;
               Basis_eig_value(no_of_basis) = D(ii,ii);
               for ell=m:1:L-1
